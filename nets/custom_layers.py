...
Trying to follow as much as possible the style/standards used in tf.contrib.layers.
"""
...
def l2_normalization(
        ...
        scope=None):
    """
    Implements L2 normalization on every feature (i.e. spatial normalization).

    Should be extended in some near future to other dimensions, providing a more
    flexible normalization framework.

    :param inputs: a 4-D tensor with dimensions [batch_size, height, width, channels].
    :param scaling: whether or not to add a post scaling operation along the dimensions
        which have been normalized.
    :param scale_initializer: An initializer for the weights.
    ...
    :param data_format: NHWC or NCHW data format.
    ...
    with variable_scope.variable_scope(
        ...
        dtype = inputs.dtype.base_type
        if data_format == 'NHWC':
            norm_dim = tf.range(inputs_rank-1, inputs_rank)     # [3]
            params_shape = inputs_shape[-1:]    # [C], it is a list
        elif data_format == 'NCHW':
            norm_dim = tf.range(1, 2)                           # [1]
            params_shape = (inputs_shape[1])    # C, it is a int
